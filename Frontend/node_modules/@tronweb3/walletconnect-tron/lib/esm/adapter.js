...
import { mainnet, nileTestnet, shastaTestnet, NETWORK_MAP } from './utils.js';
...
export class WalletConnectWallet {
    ...
    async getProvider() {
        ...
        if (!this.providerPromise) {
            ...
            this.providerPromise = UniversalProvider.init({
                ...
                relayUrl: this._options?.relayUrl,
    ...
    setupSessionListeners() {
        ...
        this.sessionHandlers.update = ({ topic, params }) => {
            ...
            let updated;
            try {
                updated = this._client?.session.get(topic);
            }
            catch (_) {
    ...
    async connect(options) {
        ...
        if (sessions.length) {
            ...
            if (!this.appKit) {
                ...
                const { network, themeMode, themeVariables, allWallets, featuredWalletIds, includeWalletIds, excludeWalletIds, customWallets, enableAnalytics, debug, ...extraAppKitConfig // Spread any additional AppKit config
                 } = this._config;
    ...
    async disconnect() {
        try {
            ...
            const topic = this._session?.topic || this.provider?.session?.topic;
            if (!topic)
                throw new ClientNotInitializedError();
            const client = this.provider?.client || this._client;
            if (!client)
                throw new ClientNotInitializedError();
